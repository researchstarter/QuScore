...
class Problem:
    ...
    def evaluate(self, x):
        temp = torch.tensor(np.clip(np.tile(self.image, (len(x), 1, 1, 1)) + np.reshape(x, (len(x),) + self.image.squeeze().shape) * self.eps, 0., 1.)).float()
        predict = self.model(temp.to(flags.device)).detach().cpu().numpy()
        p = np.copy(predict[:, self.true_label[0]])
...
def GA(pop_size, generation, length, model, hmodel, image, true_label, eps, batch_size, gradient=None, multi_fit=False):
    generation_save = np.zeros((10000,))

    pred = np.squeeze(model(torch.tensor(image)[None, ...].to(flags.device)).detach().cpu().numpy())
    r = np.argmin(pred)
    a = pred[true_label]
    pred[true_label] = 0
    b = -np.max(pred)
    ...
    adversary = MI_FGSM_ENS([hmodel], epsilon=eps,
                                iters=10, targeted=False)

    #print(image.shape)
    datas = torch.tensor(image).repeat((pop_size, 1, 1, 1))
    labels = torch.tensor(true_label).repeat(pop_size)
    adv = adversary.perturb(datas, labels).cpu().numpy()
    delta = adv - image
    #print(delta.shape)
    pop = np.reshape(delta, (pop_size, length))
